...
Здесь используется *list comprehension* при создании списка. Затем с помощью *enumerate* к элементам списка добавляются индексы. С использованием *filter* отфильтровываются элементы с нечётными индексами через *lambda*. Сумма элементов находится с помощью *sum* для списка.  
2. ***Напишите программу, которая найдёт произведение пар чисел списка. Парой считаем первый и последний элемент, второй и предпоследний и т.д.***  
